...
{
	zval_mgr self(vobj());
	table_model_.call(IRSTR.reload_key, self);	
}
...
IRow::getDataValues(zval_user attrlist)
{
	...
	if (attrlist.isArray())
	{
		if (data_.size())
		{
			result = htab_mgr::subset(data_, attrlist, true);
		}
	}
	return result;
}

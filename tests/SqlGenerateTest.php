...
class SqlGenerateTest extends PHPUnit\Framework\TestCase
{
    ...
    public function testGenEx1()
    {
        ...
        fwrite(STDERR, "\nsql generation iteration $msec ms\n");
...
EOS;
        ...
        $sql_key = __CLASS__ . '.links_sql_test';

        $sql = $cache ? $cache->set($sql_key, $ans) : null;
    }
...
EOS;
        ...
        fwrite(STDERR, "\nEx2 sql iteration $msec ms\n");
        $this->assertStringStartsWith('SELECT', $sql);
        
        $cache = Services::service('sql_cache');

        $sql_key = __CLASS__ . '.blog_sql_test';
        $sql = $cache ? $cache->set($sql_key, $ans) : null;
    }
